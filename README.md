...
좌석은 (screen_id, seat_row, seat_col) 기준으로 유니크 제약을 두었고

예매 좌석은 (schedule_id, seat_id) 기준으로 중복을 차단했습니다.
...
사용자가 예매 요청 시 BOOKING(PENDING) 생성

좌석은 BOOKING_SEAT(HELD) 상태로 점유
...
이를 통해 예매/결제 과정에서의 데이터 정합성을 보장합니다.
...
단순 CRUD 구현보다 도메인 설계와 상태 관리가 더 중요함을 체감했습니다.

실제 서비스에서는 예외 상황과 데이터 정합성 관리가 핵심이라는 점을 배웠습니다.

DB 제약조건과 애플리케이션 로직이 함께 동작해야 안정적인 서비스가 된다는 것을 경험했습니다.
