...
import type { MovieListItemAdminResponseDto } from "../../../shared/auth/types";
import './AdminMovieListPage.css';

export default function AdminMovieListPage() {
  const nav = useNavigate();
  const [items, setItems] = useState<MovieListItemAdminResponseDto[]>([]);
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const data = await adminMovieApi.getList();
      setItems(data || []);
    } catch (e: any) {
  ...
  const onCreateDraft = async () => {
    try {
      ...
      alert(e?.response?.data?.message ?? "초안 생성 실패");
    }
  };
              ...
              items.map((m) => (
                ...
                >
                  ...
                  <td className="admin-movie-list__title-cell">
                    {m.title ? m.title : <span className="admin-movie-list__muted">(제목 없음)</span>}
                  </td>
