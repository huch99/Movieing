...
const AdminTheaterListPage = () => {
    ...
    //     setLoading(true);
    //     try {
    //         const res = await adminTheaterApi.list();
    //         const data = res?.data?.data ?? res?.data; // axios 응답 방어
    ...
    //     try {
    //         const ok = window.confirm("영화관 초안을 생성할까요?");
    //         if (!ok) return;

    //         const res = await adminTheaterApi.createDraft(); // POST /api/admin/theaters/draft
    //         const data = res?.data?.data ?? res?.data;

    //         // 서버가 draft의 id만 주든, detail을 주든 대비
    //         const id = data?.theaterId ?? data?.id ?? data?.theater?.theaterId;
    //         if (!id) {
                            ...
                            filtered.map((it) => {
                                ...
                                return (
                                    <tr key={id}>
                                        <td>{id}</td>
                                        <td>
                                            <Link to={`/admin/theaters/${id}`} className="admin-theater-list__link">
                                        ...
                                        <td>
                                            <Link to={`/admin/theaters/${id}`} className="admin-theater-list__action">
                                                상세
                                            </Link>
                                        </td>
                                    </tr>
